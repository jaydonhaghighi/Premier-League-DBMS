...
# Load event data from JSON files into the database
# This function only inserts events, shots, passes, and dribbles
# Because this data is important for the queries
...
def load_event_data(file_paths, conn_params):
    for file_path in file_paths:
        try:
            ...
            print(f"Data from {file_path} have been loaded successfully.")
    ...
    for root, dirs, files in os.walk(directory_path):
        for filename in files:
            if filename.endswith('.json'):
                file_path = os.path.join(root, filename)
                json_files.append(file_path)
                if len(json_files) >= 500:  # Stop collecting file paths after the first 10
                    break
        if len(json_files) >= 500:  # Ensure that the loop exits once we have enough files
            break

    try:
        load_event_data(json_files, conn_params)
        print("First 10 JSON files loaded successfully.")
    except Exception as e:
        print(f"Failed to load data: {e}")
