...
def Q_1(conn, execution_time):
    ...
    query = """ 
            SELECT
                pl.player_name,
                AVG(es.statsbomb_xg) AS average_xg
            FROM
                players pl
            JOIN
                events e ON pl.player_id = e.player_id
            JOIN
                event_shot es ON e.event_id = es.event_id
            JOIN
                matches m ON e.team_id = m.home_team_id OR e.team_id = m.away_team_id
            JOIN
            ...
            GROUP BY
                pl.player_name
            HAVING
                COUNT(es.statsbomb_xg) > 0  -- ensures there is at least one shot
            ORDER BY
                average_xg DESC;
